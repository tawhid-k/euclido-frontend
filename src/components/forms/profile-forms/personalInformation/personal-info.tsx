...
import { patchRequest, patchFormDataRequest } from '@/@/service/api-handler/patch-manager'
...
export function ChangePhoto() {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            if (selectedPhoto && !selectedPhoto.includes(profile?.avatarPath || '')) {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        formData.append('countryCode', profile?.studentDetails.country || '')
        formData.append('stateCode', profile?.studentDetails.state || '')
        console.log(selectedFile)
        formData.append('avatar', selectedFile) // Match the API parameter
        try {
 
            const response = await patchFormDataRequest('auth/update-profile', formData)
            if (response.statusCode === 200) {
                toast.success('Image uploaded successfully!')
                //TODO: remove the console message
    ...
    const handleRemove = (event: React.MouseEvent<HTMLButtonElement>) => {
        event.preventDefault()
        // Clean up object URL if it's not from the profile
        if (selectedPhoto && !selectedPhoto.includes(profile?.avatarPath || '')) {
            URL.revokeObjectURL(selectedPhoto)
        }
