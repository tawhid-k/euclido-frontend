'use client'
import { getRequestUrl } from '@/@/service/api-handler/get-manager'

import useJobsListStore from '@/@/context/jobs/jobs-list-filter-context'

export default function JobsNavigation() {
    const links = useJobsListStore((state) => state.links)
    const clearJobss = useJobsListStore((state) => state.clearJobs)
    const setJobss = useJobsListStore((state) => state.setJobs)
    const setMeta = useJobsListStore((state) => state.setMeta)
    const meta = useJobsListStore((state) => state.meta)
    ...
    const fetchAndUpdate = async (url: string | undefined) => {
        ...
        console.log(result)
        scrollToTop()
        setJobss(result.result)
        setMeta(result.meta)
        setLinks(result.links)
    }
    if (meta?.currentPage === meta?.totalPages || !links) {
        return <></>
    } else
        return (
            <div className="flex gap-4 py-6 h-24">
                {links.previous && (
                    <button
                        ...
                        className="text-button-primary font-medium text-sm flex items-center gap-2 rounded-lg border-2 border-text-light-gray py-2 px-4 hover:border-button-primary transition ease-out duration-150"
                    >
                        <svg
                            width="14"
                            height="11"
